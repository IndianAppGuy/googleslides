  ...
  const handleMouseMove = (e) => {
    if (isResizing) {
      ...
      const newX = Math.max(
        0,
        Math.min(
          SLIDE_WIDTH - element.size.width,
      ...
      const newY = Math.max(
        0,
        Math.min(
          SLIDE_HEIGHT - element.size.height,
...
const TextElement = ({
  ...
  const handleMouseMove = (e) => {
    ...
    const newX = Math.max(
      0,
      Math.min(SLIDE_WIDTH - 100, e.clientX - dragStartPos.current.x)
    );
    const newY = Math.max(
      0,
      Math.min(SLIDE_HEIGHT - 50, e.clientY - dragStartPos.current.y)
    );
    ...
    textElements.forEach((element) => {
      ...
      const textOptions = {
        ...
        paraSpaceAfter: element.style.lineHeight
          ...
          : 0, // Convert line height to percentage
        charSpacing: element.style.letterSpacing
          ? element.style.letterSpacing * 100
          : 0, // Convert letter spacing to percentage
      };
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="text-sm text-gray-600">
        <span>
          Position: ({Math.round(element.position.x)}px,{" "}
          {Math.round(element.position.y)}px)
        </span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          onChange={(e) =>
            handleStyleChange("letterSpacing", parseInt(e.target.value))
          }
          className="w-full"
        />
      </div>
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium">
          Line Height: {element.style.lineHeight || 1}
        </label>
        <input
          type="range"
          min="10"
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          onChange={(e) =>
            handleStyleChange("transparency", parseInt(e.target.value))
          }
          className="w-full"
        />
      </div>
