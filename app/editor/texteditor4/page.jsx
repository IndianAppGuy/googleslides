...
const TextElement = ({
  ...
  const handleMouseDown = (e) => {
    ...
    // Only start dragging when clicking the border area
    const rect = elementRef.current.getBoundingClientRect();
    const isClickNearBorder = 
      e.clientX - rect.left < 10 || // left border
      rect.right - e.clientX < 10 || // right border
      e.clientY - rect.top < 10 || // top border
      rect.bottom - e.clientY < 10;  // bottom border
...
// Replace the existing exportToPPTX function with this corrected version
const exportToPPTX = async (textElements) => {
    try {
...
const SlideEditor = () => {
  ...
  const handleExport = async () => {
    const success = await exportToPPTX(textElements);
    if (success) {
